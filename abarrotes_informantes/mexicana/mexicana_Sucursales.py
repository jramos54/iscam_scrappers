...
def sucursales_abarrotes(driver,fecha):
    ...
    """
    INFORMANTE='Mexicana de Abarrotes'
    URL='https://mexicanadeabarrotes.mx/?page_id=66'
    driver.get(URL)
    ...
    soup = BeautifulSoup(html, 'html.parser')
    print(soup.prettify)
    sucursal=soup.find(id="featurecardPanel")
    print(sucursal)

    elemento_div = driver.find_element(By.CSS_SELECTOR, 'div[role="button"]')
    elemento_div.click()

    # menu_sucursales = driver.find_element(By.ID, "featurecardPanel")
    # driver.execute_script("arguments[0].removeAttribute('style');", menu_sucursales)
    ...
    buttons=driver.find_elements(By.XPATH,"//div[@tabindex='-1']")
    for button in buttons:
        menu_sucursales = driver.find_element(By.ID, "featurecardPanel")
        driver.execute_script("arguments[0].removeAttribute('style');", menu_sucursales)

        driver.execute_script("arguments[0].setAttribute('tabindex', '0');", button)
        main=soup.find(class_="page-content clearfix the-content-parent")
        sucursal=main.find(id="featurecardPanel")
        sucursal_text=sucursal.text
        print(sucursal_text)
